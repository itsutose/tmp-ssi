...
- **文書検索系**: `POST /search`
- **チェック系**: `POST /compliance/check`, `POST /term/check`, `POST /expression/check`
- **処理状態確認**: `GET /status/check`
...
- **レスポンスタイム**
  - 文書検索：15秒以内
  - チェック機能：120秒以内（Step Functionでの非同期処理）
  - ファイルアップロード：120秒以内
  - Step Functionでワークフローのステータス管理、ポーリング対応
